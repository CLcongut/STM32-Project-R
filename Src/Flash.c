...
{
  uint64_t flash_read_buf;
  Flash_Read(FLASH_TYPEPROGRAM_HALFWORD, FLASH_START_ADDR, &flash_read_buf, 1); // 从指定页的地址读FLASH
  cur_rail_state = flash_read_buf;
}

void MY_Flash_Write(void)
{
  uint64_t flash_write_buf;
  flash_write_buf = cur_rail_state;
  Flash_Write(FLASH_TYPEPROGRAM_HALFWORD, FLASH_START_ADDR, &flash_write_buf, 1);
}