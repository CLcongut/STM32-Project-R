...
/* Private macro -------------------------------------------------------------*/
/* USER CODE BEGIN PM */
uint8_t char1[] = "monitor";
uint8_t char2[] = "alarm";
uint8_t char3[] = "command error:please input again";
/* USER CODE END PM */
...
//        HAL_UART_Transmit_DMA(&huart1, (uint8_t*)RxBuffer, RecCount);
        if(memcmp(RxBuffer, char1, RecCount) == 0){
            main_LED_dsp_state.flag1_LED_state_dsp = 1;
        }
        else if(memcmp(RxBuffer, char2, RecCount) == 0){
            main_LED_dsp_state.flag2_beep_state = 1;
        }
        else{
            HAL_UART_Transmit_DMA(&huart1, char3, sizeof(char3));
        }
        RecCount = 0;
